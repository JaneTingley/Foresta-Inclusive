...
//HEAT SYSTEM
 ...
 unsigned long H_currentTime = millis();
  ...
  //this will input a new heat value into the system, every 5 minutes  
...
//WIND SYSTEM
 ...
 unsigned long w_currentTime = millis();
  ...
  if (w_currentTime - w_previousTime_update >= w_update_value) {
    ...
    wind_counter ++; //get new wind value, remove if shfitr.io is being used 
    float updated_wind_speed = speed_array[wind_counter] ; //extract, shiftr.io function would be placed here 
    update_wind_system(updated_wind_speed);
    }  
