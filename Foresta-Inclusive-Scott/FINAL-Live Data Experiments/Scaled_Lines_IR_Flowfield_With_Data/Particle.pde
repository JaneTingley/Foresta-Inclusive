class Particle {
  ...
  void avoidIR(float sensorPositionX) {
    ...
    float rawMouseSpeed = dist(mouseX, mouseY, pmouseX, pmouseY);
    float smoothedSpeedToRadius = map(rawMouseSpeed, 1, 40, 30, 60);
      float mappedArrayToWidth = map(sensorPositionX, 0, 3, 0, width);

    if (dist(position.x, position.y, mappedArrayToWidth, width/2) < 150) {
      PVector irSensor = new PVector(mappedArrayToWidth, width/2);
      irSensor.sub(position); // gets the position of the particle in relation to the mouse
      acceleration = irSensor;

      irSensor.setMag(random(-3, -1)); //sets the strength of the avoidance (negatives numbers repel, positive numbers attract);
      color newColor = lerpColor(particleColor, color(#f093fb), sensorTemp+0.45);
      particleColor = newColor;
    } else {
      if (particleSizeIncreaser > 1) {
  ...
  void irChecker(int [] sensorArray) {
    ...
    float ballx = map(irSensorTripped, 0, 3, 0, width);
    avoidIR(irSensorTripped);
    ellipse(ballx, height/2, 50, 50);
  }


  void show(int timeIn, int [] sensorArrayIn) {
    changeColor(timeIn);
   // avoidUser();
